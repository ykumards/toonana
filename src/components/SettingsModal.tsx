...
export function SettingsModal({ open, onClose, onSaved }: SettingsModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative w-full max-w-lg rounded-lg bg-slate-900 shadow-lg border border-slate-700">
        ...
        <div className="p-5 space-y-5">
          {loading ? (
            ...
            <>
              <div>
                <label className="block text-sm font-medium mb-1 text-slate-300">Gemini API Key</label>
                <input
                  type="password"
                  value={settings.gemini_api_key || ""}
                  onChange={(e) => setSettings(s => ({ ...s, gemini_api_key: e.target.value }))}
                  placeholder="Paste your Gemini API key"
                  className="w-full px-3 py-2 border border-slate-700 rounded-md bg-slate-800 text-white placeholder:text-slate-500"
                />
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium mb-1 text-slate-300">Ollama Base URL</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-sm font-medium mb-1 text-slate-300">Default Ollama Model</label>
                  <div className="flex gap-2">
                    <select
                      value={settings.default_ollama_model || ""}
                      onChange={(e) => setSettings(s => ({ ...s, default_ollama_model: e.target.value }))}
                      className="flex-1 px-3 py-2 border border-slate-700 rounded-md bg-slate-800 text-white placeholder:text-slate-500"
                    >
                      <option value="" className="bg-slate-800 text-white">Select a model…</option>
                      {models.map(m => (
                        <option key={m} value={m} className="bg-slate-800 text-white">{m}</option>
                      ))}
                    </select>
                    <button onClick={handleRefreshModels} className="px-3 py-2 border border-slate-700 rounded-md bg-slate-800 text-white hover:bg-slate-700">Refresh</button>
                  </div>
                  <div className="mt-1 text-xs text-slate-400">{statusText}</div>
                </div>
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium mb-1 text-slate-300">Nano-Banana Base URL</label>
                  <input
                    type="text"
                    ...
                    className="w-full px-3 py-2 border border-slate-700 rounded-md bg-slate-800 text-white placeholder:text-slate-500"
                  />
                  <div className="mt-1 text-xs text-slate-400">Optional. Leave blank to use Gemini for images.</div>
                </div>
                <div>
                  <label className="block text-sm font-medium mb-1 text-slate-300">Nano-Banana API Key</label>
                  <input
                    type="password"
                    ...
                    className="w-full px-3 py-2 border border-slate-700 rounded-md bg-slate-800 text-white placeholder:text-slate-500"
                  />
                  <div className="mt-1 text-xs text-slate-400">Only required if your Nano‑Banana server enforces it.</div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-sm font-medium mb-1 text-slate-300">Top P</label>
                  <input
                    type="number"
                    step="0.05"
                    min={0}
        ...
        <div className="px-5 py-4 border-t border-slate-700 flex justify-end gap-2">
          <button onClick={onClose} className="px-4 py-2 rounded-md border border-slate-700 bg-slate-800 text-white hover:bg-slate-700">Cancel</button>
